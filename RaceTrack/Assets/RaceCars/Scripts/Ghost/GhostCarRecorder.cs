...
public class GhostCarRecorder : MonoBehaviour
{
    ...
    {
        if (GameManager.instance.GetGameStates() == GameStates.running) StartCoroutine(RecordCarPosition());
        if (GameManager.instance.GetGameStates() == GameStates.raceOver) StartCoroutine(SaveCarPosition());
    }
